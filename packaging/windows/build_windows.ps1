...
$packageName = "sus-companion-1.0.0-rc.2-windows-$env:PROCESSOR_ARCHITECTURE".ToLowerInvariant()
...
Copy-Item "dist/$packageName/release-manifest.json" "dist/$packageName-release-manifest.json"
Copy-Item "dist/$packageName/SHA256SUMS" "dist/$packageName-SHA256SUMS"
Compress-Archive -Path "dist/$packageName" -DestinationPath "dist/$packageName.zip" -CompressionLevel Optimal
$archiveHash = (Get-FileHash "dist/$packageName.zip" -Algorithm SHA256).Hash.ToLowerInvariant()
Set-Content -Path "dist/$packageName.zip.sha256" -Encoding Ascii -Value "$archiveHash  $packageName.zip"
