...
pyz=PYZ(a.pure)
exe=EXE(pyz,a.scripts,[],exclude_binaries=True,name='sus-companion',console=False,version=str(root/'packaging/windows/version_info.txt') if platform.system()=='Windows' else None)
package_name=os.environ.get('SUS_ADB_PACKAGE_NAME',f"sus-companion-1.0.0-rc.1-{platform.system().lower()}-{platform.machine().lower()}")
