...
`manifest.json` declares a stable lowercase `plugin_id`, semantic `version`, descriptive metadata, `plugin_api_version`, portable `entry_point`, requested capabilities, and contributed components. Use `1.0` for legacy presentation or `1.1` for interactive contracts; unsupported future versions are blocked. Paths must be package-relative and may not contain `..`.
