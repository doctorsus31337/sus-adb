...
class MetadataTests(unittest.TestCase):
 def test_injected_metadata_is_deterministic(self):
  value=create_metadata(version="1.0.0-rc.2",repository_revision="abc1234567890",repository_ref="release/1.0.0-rc.2",build_timestamp="2026-01-01T00:00:00Z",build_channel="rc",platform_name="TestOS",architecture="test64",python_version="3.13")
  self.assertIsInstance(value,AppMetadata);self.assertEqual(value.release_channel,"rc");self.assertIn("SUS Companion",value.display_version);self.assertIn("1.0.0-rc.2",value.display_version);self.assertEqual(value.descriptor,"Android Security & Recovery Workstation");self.assertEqual(value.short_revision,"abc123456789");self.assertIn("Branch/ref: release/1.0.0-rc.2",value.build_details);self.assertIn("Build channel: rc",value.build_details);self.assertEqual(value.configuration_schema_version,4)
