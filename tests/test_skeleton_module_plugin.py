...
class T(unittest.TestCase):
 def test_zero_capability_disabled_documented_noop(self):
  manifest=json.loads((SOURCE/"manifest.json").read_text());self.assertFalse(manifest["enabled"]);self.assertEqual(manifest["requested_capabilities"],[]);self.assertEqual(manifest["contributed_components"],[]);self.assertTrue(all((SOURCE/name).is_file() for name in ("README.md","TUTORIAL.md","ARCHITECTURE.md","EXERCISES.md","TROUBLESHOOTING.md","CHECKLIST.md")));plugin=M.Plugin();self.assertTrue(plugin.validate().ok);self.assertEqual(plugin.activate(object()),());plugin.deactivate();self.assertIsNone(plugin.api)
 def test_full_lifecycle_and_copy_new_id_without_core_edit(self):
  with tempfile.TemporaryDirectory() as d:
   store=PluginStore(Path(d)/"store");manager=PluginManager(store,PluginTrustStore(store.root/"state/trust.json"),ContributionRegistry());self.assertTrue(manager.install(SOURCE).ok);pid="susadb.skeleton-module";self.assertTrue(manager.approve(pid).ok);self.assertTrue(manager.enable(pid).ok);self.assertTrue(manager.load(pid).ok);self.assertFalse(manager.registry.list());self.assertTrue(manager.unload(pid).ok);self.assertTrue(manager.uninstall(pid,True).ok)
