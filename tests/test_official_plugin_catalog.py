...
class T(unittest.TestCase):
 ...
 def test_seven_valid_deterministic_inactive_catalog_items(self):
  a=OfficialPluginCatalog(OFFICIAL).list();b=OfficialPluginCatalog(OFFICIAL).list();self.assertEqual(len(a),7);self.assertTrue(all(v.valid and not v.installed and not v.manifest.enabled for v in a));self.assertEqual([v.package_digest for v in a],[v.package_digest for v in b])
