...
class T(unittest.TestCase):
 ...
 def test_four_valid_deterministic_inactive_catalog_items(self):
  a=OfficialPluginCatalog(OFFICIAL).list();b=OfficialPluginCatalog(OFFICIAL).list();self.assertEqual(len(a),4);self.assertTrue(all(v.valid and not v.installed and not v.manifest.enabled for v in a));self.assertEqual([v.package_digest for v in a],[v.package_digest for v in b])
 def test_install_trust_enable_load_unload_uninstall_are_separate(self):
  with tempfile.TemporaryDirectory() as d:
   m=self.manager(d);item=m.official()[0];self.assertFalse(m.list());self.assertTrue(m.install_official(item.manifest.plugin_id,item.package_digest).ok);self.assertFalse(m.records[item.manifest.plugin_id][2].enabled);self.assertFalse(m.load(item.manifest.plugin_id).ok);self.assertTrue(m.approve(item.manifest.plugin_id,()).ok if not item.manifest.requested_capabilities else m.approve(item.manifest.plugin_id,item.manifest.requested_capabilities,True).ok);self.assertTrue(m.enable(item.manifest.plugin_id).ok);self.assertFalse(m.registry.list());self.assertTrue(m.load(item.manifest.plugin_id).ok);self.assertTrue(m.unload(item.manifest.plugin_id).ok);self.assertFalse(m.registry.list());self.assertTrue(m.uninstall(item.manifest.plugin_id,True).ok)
