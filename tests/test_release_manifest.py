...
class ReleaseManifestTests(unittest.TestCase):
 ...
 def test_checksum_helper(self):
  ...
  with tempfile.TemporaryDirectory() as d:
   p=Path(d)/"artifact";p.write_bytes(b"abc");manifest=module.generate(Path(d));self.assertEqual(manifest["files"][0]["path"],"artifact");self.assertTrue((Path(d)/"SHA256SUMS").exists())
