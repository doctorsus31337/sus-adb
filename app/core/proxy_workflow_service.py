...
class ProxyWorkflowService:
    ...
    @staticmethod
    def troubleshooting():return {"device-unreachable":"Use a host address reachable from the selected device; 127.0.0.1 means the device itself.","proxy-not-listening":"Start the selected proxy manually and verify only its configured listener.","missing-mapping":"Review explicit adb forward/reverse mappings.","certificate":"Install and trust the proxy CA manually only when authorized.","pinning":"Proxy setup does not bypass TLS pinning. Open Advanced Command Reference or Script Studio and review instrumentation explicitly."}
