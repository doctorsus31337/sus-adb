...
class ApkLaboratoryPanel(ctk.CTkFrame):
 ...
 def __init__(self,parent,theme,services,log,confirm):
  ...
  for n,t in self.tabs.items():t.configure(fg_color=theme["bg"]);t.grid_rowconfigure(0,weight=1);t.grid_columnconfigure(0,weight=1);v=ReadOnlyTextView(t,initial_text=f"{n}\n\nEvery operation is previewed and explicitly confirmed. Original artifacts are immutable.",fg_color=theme["terminal_bg"],text_color=theme["terminal_text"],border_width=1,border_color=theme["border"],wrap="word",scrollbar_button_color=theme["gold_dark"],scrollbar_button_hover_color=theme["red_hover"]);v.grid(row=0,column=0,sticky="nsew",padx=6,pady=6);self.views[n]=v
