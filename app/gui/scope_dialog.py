...
class ScopeDialog(ctk.CTkToplevel):
    def __init__(self,parent,theme,scope=None,device=None,target=None,on_save=None):
        ...
        self.message=ctk.CTkLabel(self,text="Authorization is mandatory before a session can start.",text_color=theme["error"],wraplength=600);self.message.grid(row=2,column=0,sticky="ew",padx=12)
    ...
    def save(self):
        ...
        except ValueError:self.message.configure(text="PID must be an integer.");return
        ...
        if not validation.valid:self.message.configure(text="; ".join(validation.errors));return
