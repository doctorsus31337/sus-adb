...
class EnvironmentDiagnosticsWindow(ctk.CTkToplevel):
    def __init__(self,parent,theme,records,startup_report=""):
        ...
        environment=self.tabs.add("Environment");startup=self.tabs.add("Startup");
        for page in (environment,startup):page.grid_columnconfigure(0,weight=1);page.grid_rowconfigure(0,weight=1)
        view=ctk.CTkTextbox(environment,fg_color=theme["terminal_bg"],text_color=theme["text"],wrap="word");view.grid(row=0,column=0,sticky="nsew",padx=6,pady=6);view.insert("1.0","\n\n".join(f"{'READY' if r.available else 'MISSING'} · {r.name} · {'Required' if r.required else 'Optional'}\n{r.version or r.path}\n{r.guidance}" for r in records));view.configure(state="disabled")
        self.startup_view=ctk.CTkTextbox(startup,fg_color=theme["terminal_bg"],text_color=theme["text"],wrap="word");self.startup_view.grid(row=0,column=0,sticky="nsew",padx=6,pady=6);self.startup_view.insert("1.0",startup_report or "Startup timing becomes available after the responsive Console shell is created.");self.startup_view.configure(state="disabled")
