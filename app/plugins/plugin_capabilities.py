...
PRIVACY_SENSITIVE=frozenset(("read-device-logs",))
CAPABILITY_CAUTIONS={"read-device-logs":"Device logs may contain identifiers, paths, messages, tokens, account information, and application data. This approval permits only the bounded host-owned Logcat façade."}
...
class CapabilityPolicy:
    ...
    def check(self,capability,session=None):
        ...
        caution="High-impact trusted-code capability." if capability in HIGH_IMPACT else CAPABILITY_CAUTIONS.get(capability)
        return PermissionResult(True,capability,caution=caution)
